...
type ContractStats = { total: number; expired: number; critical: number; warning: number; safe: number }
export type KontrakInitial = {
  contracts: ContractRow[]
  total: number
  loadError: boolean
  stats: ContractStats
  cabangOptions: { code: string; label: string; display?: string }[]
...
export function KontrakClient({ initial }: { initial: KontrakInitial }) {
  const searchParams = useSearchParams()
  const router = useRouter()
  const [contracts, setContracts] = useState<ContractRow[]>(initial.contracts)
  const [total, setTotal] = useState(initial.total)
  const [stats, setStats] = useState<ContractStats>(initial.stats)
  // Data awal diseed dari Server Component → tidak ada flash skeleton.
  ...
  const [cabangOptions] = useState<{ code: string; label: string; display?: string }[]>(initial.cabangOptions)
  ...
  // Render pertama memakai data seed dari server → lewati fetch awal.
          ...
          <div className="flex flex-wrap gap-6 px-4 py-4 rounded-md bg-muted/50 border border-border items-end">
            <div className="space-y-2">
              ...
              >
                ...
                {cabangOptions.map((c) => <option key={c.code} value={c.code}>{c.display ?? c.label}</option>)}
              </NativeSelect>
            </div>
            <div className="space-y-2">
