  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Riwayat perubahan data sistem ATMS
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-primary rounded-lg p-4 flex items-center gap-4 shadow-sm">
          <div className="h-8 w-8 rounded-full bg-card/20 flex items-center justify-center shrink-0">
            <ClockCounterClockwiseIcon className="h-4 w-4 text-white" />
          </div>
          <div>
            <p className="text-xl font-bold text-white leading-snug">{total}</p>
        ...
        <div className="bg-card border border-border rounded-lg p-4 flex items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="text-xl font-bold text-foreground leading-snug">{createCount}</p>
        ...
        <div className="bg-card border border-border rounded-lg p-4 flex items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="text-xl font-bold text-foreground leading-snug">{updateCount}</p>
        ...
        <div className="bg-card border border-border rounded-lg p-4 flex items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="text-xl font-bold text-foreground leading-snug">{deleteCount}</p>
...
function ActionBadge({ action }: { action: string }) {
  const map: Record<string, { label: string; className: string; icon: React.ReactNode }> = {
    CREATE: { label: 'Buat', className: 'bg-green-100 text-green-700', icon: <PlusCircle size={12} /> },
    UPDATE: { label: 'Ubah', className: 'bg-accent text-primary', icon: <Pencil size={12} /> },
    DELETE: { label: 'Hapus', className: 'bg-red-100 text-red-700', icon: <Trash size={12} /> },
  }
  const style = map[action] ?? { label: action, className: 'bg-muted text-foreground/70', icon: <PulseIcon size={12} /> }
  return (
    <span className={cn('inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold', style.className)}>
      {style.icon}
      {style.label}
    </span>
  )
