...
function calculateEndDate(posisi: string, startDate: string) {
  ...
  const monthsToAdd = posisi.toLowerCase().includes('admin') ? 3 : 6
  const endDate = addMonths(start, monthsToAdd)
  return format(endDate, 'yyyy-MM-dd')
}

describe('Logika Bisnis Kontrak PT. Multi Makmur', () => {
  it('harus mengatur kontrak 3 bulan jika jabatan adalah ADMINISTRATOR', () => {
    const tglMulai = '2024-01-01'
    const tglSelesai = calculateEndDate('ADMINISTRATOR', tglMulai)
    
    // Ekspektasi: 2024-01-01 + 3 bulan = 2024-04-01
    expect(tglSelesai).toBe('2024-04-01')
  })

  it('harus mengatur kontrak 6 bulan untuk jabatan selain ADMINISTRATOR (Contoh: SALES EXECUTIVE)', () => {
    const tglMulai = '2024-01-01'
    const tglSelesai = calculateEndDate('SALES EXECUTIVE', tglMulai)
