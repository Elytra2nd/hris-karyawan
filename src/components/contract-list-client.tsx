    ...
    <Card className="border-none shadow-sm overflow-hidden">
      ...
      <CardContent className="p-0">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left font-sans" aria-label="Riwayat perjalanan kontrak">
            <thead className="bg-muted/50 border-b border-border/60">
              <tr>
                <th scope="col" className="px-6 py-2 font-bold text-foreground/80 uppercase text-xs tracking-tighter italic">Jabatan</th>
                <th scope="col" className="px-6 py-2 font-bold text-foreground/80 uppercase text-xs tracking-tighter italic text-center">Aksi</th>
                <th scope="col" className="px-6 py-2 font-bold text-foreground/80 uppercase text-xs tracking-tighter italic">Mulai</th>
                <th scope="col" className="px-6 py-2 font-bold text-foreground/80 uppercase text-xs tracking-tighter italic">Selesai</th>
                <th scope="col" className="px-6 py-2 font-bold text-foreground/80 uppercase text-xs tracking-tighter text-center">Status</th>
              </tr>
            </thead>
            <tbody>
              {contracts.map((contract, index) => {
                ...
                return (
                  <tr key={contract.id} className="border-b border-border/40 hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      {index === 0 && !isExpired ? (
                        <Badge className="bg-emerald-500 uppercase text-xs font-black" aria-label="Status kontrak: aktif">Running</Badge>
                      ) : (
                        <Badge variant="outline" className="text-muted-foreground/70 uppercase text-xs font-bold" aria-label="Status kontrak: selesai">Closed</Badge>
                      )}
                    </td>
                  </tr>
                );
