...
// Sekarang dynamic import dengan ssr: false diperbolehkan di sini
const PDFButton = dynamic(() => import('@/components/pdf-button'), { 
  ssr: false,
  loading: () => <span className="text-[10px] opacity-50 font-bold uppercase">Loading...</span>
});

export function ContractList({ employee, contracts }: { employee: any, contracts: any[] }) {
  return (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left font-sans">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="px-6 py-3 font-bold text-slate-700 uppercase text-[10px] tracking-tighter">Jabatan</th>
                <th className="px-6 py-3 font-bold text-slate-700 uppercase text-[10px] tracking-tighter text-center">Aksi</th>
                <th className="px-6 py-3 font-bold text-slate-700 uppercase text-[10px] tracking-tighter">Mulai</th>
                <th className="px-6 py-3 font-bold text-slate-700 uppercase text-[10px] tracking-tighter">Selesai</th>
